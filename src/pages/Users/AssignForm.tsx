...
export default function AssignForm() {
  ...
  const [saving, setSaving] = useState(false); // save btn
  const [deletingId, setDeletingId] = useState<number | null>(null); // per row delete
  const [expoLoading, setExpoLoading] = useState(false); // expo dropdown loading
  ...
  // ---------------- Fetch Expo List (Industry-wise) ----------------
  const fetchExposByIndustry = async (indId: string) => {
    if (!indId) {
      setExpoList([]);
      return;
    }
    ...
    try {
      ...
      // API sample:
      // data: [{ expoid: 17, exponame: "Optic Expo" }, ...]
      const raw = res.data?.data || [];

      const mapped: ExpoItem[] = raw.map((x: any) => ({
        id: Number(x.expoid ?? x.Expoid ?? x.id),
        name: String(x.exponame ?? x.Exponame ?? x.name ?? ""),
      }));

      // OPTIONAL: remove duplicate expo by id
      const uniq = Array.from(new Map(mapped.map((e) => [e.id, e])).values());

      setExpoList(uniq);
    } catch (err: any) {
      console.error(err);
      toast.error("Industry wise expo fetch failed");
      setExpoList([]);
    } finally {
  ...
  // ✅ When industry changes -> reset expo + load industry-wise expos
  useEffect(() => {
    setExpoId(""); // reset selected expo
    ...
    <div className="p-6">
      ...
      <div className="flex gap-6">
        ...
        <div className="w-1/3 bg-white p-6 rounded-xl shadow">
          ...
          {/* Expo Dropdown (Industry wise) */}
          <label className="font-medium">Select Expo</label>
          <select
            value={expoId}
            onChange={(e) => setExpoId(e.target.value)}
            disabled={!industryId || expoLoading}
          ...
          <table className="w-full border-collapse">
            ...
            <tbody>
              {loading ? (
                ...
                assignedList.map((item) => (
                  <tr key={item.expo_assign_user_id} className="border-b">
                    <td className="p-2 border">{item.industryname || "-"}</td>
                    <td className="p-2 border">{item.exponame || "-"}</td>
                    <td className="p-2 border text-center">
                      <button
                        disabled={deletingId === item.expo_assign_user_id}
                        onClick={() => {
