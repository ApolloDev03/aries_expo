...
const LeadManagement = () => {
  ...
  const [visitorCategoryList, setVisitorCategoryList] = useState<VisitorCategoryItem[]>([]);
  ...
  const fetchIndustries = async (): Promise<IndustryItem[]> => {
    try {
      ...
      setIndustryList(finalList);
      return finalList;
    } catch (err: any) {
      console.error(err);
      setIndustryList([]);
      toast.error(err?.message || "Industry fetch failed");
      return [];
    } finally {
  ...
  const fetchVisitorCategories = async (
    ...
    try {
      ...
      const res = await axios.post(`${apiUrl}/visitor-category/index`, {
        industry_id: String(selectedIndustryId || ""),
      });

      if (!res.data?.status) {
        throw new Error(getApiErrorMessage(res.data, "Visitor category fetch failed"));
      }
      ...
      setVisitorCategoryList(finalList);
      return finalList;
    } catch (err: any) {
      console.error(err);
      setVisitorCategoryList([]);
  ...
  useEffect(() => {
    if (formData.industry_id) {
      fetchVisitorCategories(formData.industry_id);
    } else {
      setVisitorCategoryList([]);
      setFormData((prev) => ({
        ...prev,
  ...
  const fetchUniqueVisitor = async () => {
    ...
    if (!formData.visitor_category_id) {
      toast.error("Please select visitor category");
      return;
    }
    ...
    try {
      ...
      const payload = {
        ...
        industry_id: String(formData.industry_id),
  ...
  //       next_followup_date: formatDateTimeForApi(formData.next_follow_up),
  ...
  //     // IMPORTANT:
  //     // Do NOT call Get_unique_visitor here again.
  //     // Use create API response data as next record.
  //     const nextSet = setNextRecordFromResponse(res.data?.data);

  //     if (!nextSet) {
  //       // if response has no next visitor data, close form or keep as needed
  //       setShowForm(false);
  //       setCurrentVisitor(null);
  ...
  const selectDisabledClass =
    "w-full bg-gray-100 border border-gray-300 rounded-lg px-4 py-3 outline-none disabled:opacity-70 disabled:cursor-not-allowed";

  return (
    <div className="min-h-screen bg-[#f3f4f6] flex items-center justify-center px-4 py-10 text-black font-sans">
      {!showForm ? (
          ...
          <div className="space-y-5">
            ...
            <div>
              ...
              <select
                ...
                onChange={handleChange}
                disabled={!formData.industry_id || loadingVisitorCategory}
                className={
                  !formData.industry_id || loadingVisitorCategory
                    ? selectDisabledClass
                    : inputClass
                }
              >
                <option value="">
                  {!formData.industry_id
                    ...
                    : loadingVisitorCategory
                      ? "Loading visitor categories..."
                      : "Select Visitor Category"}
                </option>
                {visitorCategoryList.map((category) => (
          ...
          <div className="space-y-6">
            <div>
              ...
              >
                ...
                <option value="to-be-call-back">To Be Call Back</option>
              </select>
            </div>
